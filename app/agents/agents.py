...
from app.memory.store import set_last_ticket, set_last_claim, get_last_ticket, get_last_claim
...
class ItAgent(BaseAgent):
    ...
    def handle(self, query: str, context_items: List[Tuple[str, float]], graph_hints: List[str]) -> Dict[str, Any]:
        low = query.lower()
        # Tool intents
        if 'what' in low and 'ticket' in low and ('last' in low or 'just' in low):
            last = get_last_ticket('global') or get_last_ticket('') or get_last_ticket('default')
            if last:
                return {'dept': self.dept, 'action': 'recall_ticket', 'result': last}
...
class FinanceAgent(BaseAgent):
    ...
    def handle(self, query: str, context_items: List[Tuple[str, float]], graph_hints: List[str]) -> Dict[str, Any]:
        ...
        if ('what' in low or 'which' in low) and ('claim' in low or 'reimburse' in low) and ('last' in low or 'just' in low):
            last = get_last_claim('global') or get_last_claim('') or get_last_claim('default')
            if last:
                return {'dept': self.dept, 'action': 'recall_claim', 'result': last}
